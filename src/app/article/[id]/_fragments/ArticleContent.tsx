...
const ArticleContent = ({ content, tags }: ArticleContentProps) => {
    return (
        <Flex.VStack className={`w-full px-8 min-h-full h-fit text-${textColor.normal} xl:max-w-[1200px] xl:px-0`}>
            ...
            <Flex.HStack className='gap-2 my-4 text-gray-400 flex-wrap'>
                {
                    tags.map((item, idx) => {
                        return (
                            <Flex.Center
                                key={`${item}+${idx}`}
                                className='w-fit h-fit px-3 py-1 bg-gray-500 text-white rounded-full inline-flex text-sm'
                            >
                                {`${item}`}
                            </Flex.Center>
                        );
